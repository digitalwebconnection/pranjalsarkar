...
export default function JudgementSection() {
  return (
    <section
      id="curriculum"
      className="relative py-4 md:py-14 overflow-hidden border-b border-white/8 bg-[#000001]"
    >
      {/* Background Decorative Gradients */}
      {/* Background Decorative Wavy Dots & Glow */}
      <div
        className="absolute top-0 right-0 w-150 h-150 md:w-225 md:h-225 opacity-80 pointer-events-none z-0"
        style={{
          backgroundImage:
            "radial-gradient(circle, #0080C7 2px, transparent 2px)",
          backgroundSize: "32px 32px",
          transform:
            "perspective(800px) rotateX(60deg) rotateY(-20deg) skewX(20deg) scale(1.2) translateX(10%) translateY(-10%)",
          WebkitMaskImage:
            "radial-gradient(circle at 70% 30%, black 10%, transparent 60%)",
          maskImage:
            "radial-gradient(circle at 70% 30%, black 10%, transparent 60%)",
        }}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
        ...
        <div className="max-w-6xl mx-auto text-center justify-center mb-0">
          ...
          <p className="text-gray-300 text-sm md:text-[15px] lg:text-[17px] leading-8 max-w-6xl mx-auto font-sans">
            Every week builds one executive capability through simulation, live
            instruction, AI practice and independent study.
          </p>
        </div>
          ...
          {stages.map((s, i) => (
            <ScrollStackItem key={i}>
              <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-8 lg:gap-12 px-4 py-6 md:p-8 rounded-xl bg-[#0a0c10] border border-white/10 border-l-[3px] border-l-[#0044cc] shadow-[inset_30px_30px_80px_-20px_rgba(0,68,204,0.4),0_15px_40px_rgba(0,0,0,0.5)] hover:shadow-[inset_30px_30px_100px_-20px_rgba(0,68,204,0.5),0_20px_50px_rgba(0,0,0,0.6)] hover:border-white/20 hover:border-l-[#0044cc] h-auto min-h-100 flex-col justify-between transition-all duration-500 relative overflow-hidden group">
                ...
                <div
                  className="absolute bottom-0 left-0 w-70 h-full max-h-62.5 pointer-events-none opacity-15 group-hover:opacity-30 transition-opacity duration-700"
                  style={{
                    WebkitMaskImage:
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-x-10 md:gap-y-8">
                      {s.curriculum.map((cur, idx) => (
                        <div
                          key={idx}
                          className="flex flex-col bg-[#0f1624] md:bg-transparent p-5 md:p-0 rounded-xl md:rounded-none border border-[#0080C7]/20 md:border-transparent shadow-lg md:shadow-none relative"
                        >
                          ...
                          <ul className="space-y-3">
                            {cur.items.map((item, iIdx) => (
                              <li
                                key={iIdx}
                                className="flex items-start gap-2.5 text-gray-300 text-[14px] leading-relaxed font-sans"
                              >
                                <CheckCircle
                                  size={15}
                                  strokeWidth={2.5}
                                  className="text-[#0080C7] shrink-0 mt-0.75 drop-shadow-[0_0_5px_rgba(0,128,199,0.5)]"
                                />
                                <span>{item}</span>
                              </li>
                            ))}
